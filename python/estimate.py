...
if __name__ =='__main__':
        ...
        for n in range(2,8):
            ...
            for i in range(NUM_CLASSES):
                if result[0][i] > maxx1:
                    maxx1 = result[0][i]
                    maxx1_index = i
                elif (result[0][i]>maxx2) and (result[0][i]<=maxx1):
                    maxx2 = result[0][i]
                    maxx2_index = i
                elif (result[0][i]>maxx3) and (result[0][i]<=maxx2):
                    maxx3 = result[0][i]
                    maxx3_index = i
            license_num = license_num + LETTERS_DIGITS[maxx1_index]
    x = "车牌编号是: 【%s】" % license_num
    text = x
    im = Image.new("RGB", (300, 300), (255, 255, 255))
    font = pygame.font.Font(os.path.join("python/fonts/", "msyh.ttc"), 34)
    rtext = font.render(text, True, (0, 0, 0), (255, 255, 255))
    pygame.image.save(rtext, "python/result/4.jpg")
    line = Image.open("python/result/4.jpg")
    im.paste(line, (10, 5))
    src7 = cv2.imread("python/result/4.jpg")
    cv2.imshow("result", src7)
    cv2.waitKey(0)
    y = license_num
    text = y
    im = Image.new("RGB", (300, 300), (255, 255, 255))
    ...
    line = Image.open("python/result/3.jpg")
    im.paste(line, (10, 5))
